...
import { PoolCreated } from "../../types/amm-v3/Factory/Factory";
import { AMMFactory } from "../../types/amm-v3/schema";
import { Bundle, Pool, Token } from "../../types/amm-v3/schema";
import { Pool as PoolTemplate } from "../../types/amm-v3/templates";
...
import { ADDRESS_ZERO, ONE_BI, ZERO_BD, ZERO_BI } from "../constant";
...
export function handlePoolCreatedHelper(
  ...
  if (token0 === null) {
    ...
    token0.untrackedVolume = ZERO_BD;
    token0.tradeVolume = ZERO_BD;
    token0.tradeVolumeBridge = ZERO_BD;
    token0.volumeUSD = ZERO_BD;
    token0.volumeUSDBridge = ZERO_BD;
    token0.traderCount = ZERO_BI;
  ...
  if (token1 === null) {
    ...
    token1.untrackedVolume = ZERO_BD;
    token1.tradeVolume = ZERO_BD;
    token1.tradeVolumeBridge = ZERO_BD;
    token1.volumeUSD = ZERO_BD;
    token1.volumeUSDBridge = ZERO_BD;
    token1.traderCount = ZERO_BI;
