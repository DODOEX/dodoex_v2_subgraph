...
import {
  ...
  Claim,
  CP,
} from "../../types/dodoex/templates/CP/CP";
import {
  updateCrowdPoolingDayData,
  updateCrowdPoolingHourData,
} from "./dayUpdates";
...
export function handleBid(event: Bid): void {
  ...
  //用户信息
  let bidPositionID = event.params.to
    .toHexString()
    .concat("-")
  ...
  bidPosition.shares = event.params.amount
    .minus(event.params.fee)
    .toBigDecimal()
    .plus(bidPosition.shares);

  //交易记录
  let bidHistoryID = event.transaction.hash
    .toHexString()
    .concat("-")
...
export function handleCancel(event: Cancel): void {
  ...
  bidPosition.shares = bidPosition.shares.minus(
...
export function handleClaim(event: Claim): void {
  //用户信息
  let bidPositionID = event.params.user
    .toHexString()
    .concat("-")
    .concat(event.address.toHexString());
  let bidPosition = BidPosition.load(bidPositionID);
