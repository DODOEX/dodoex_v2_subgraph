...
} from "../../types/dodoex/DVMFactory/DVM";
...
export let dvmFactoryContract = DVMFactory.bind(
  Address.fromString(DVM_FACTORY_ADDRESS)
);
export let dppFactoryContract = DPPFactory.bind(
  Address.fromString(DPP_FACTORY_ADDRESS)
);
export let classicFactoryContract = DODOZooContract.bind(
  Address.fromString(CLASSIC_FACTORY_ADDRESS)
);
...
export function createTokenByCall(
  address: Address,
  call: ethereum.Call
): Token {
  let token = Token.load(address.toHexString());
...
export function updateVirtualPairVolume(
  event: OrderHistoryV2,
  dealedFromAmount: BigDecimal,
  dealedToAmount: BigDecimal,
  volumeUSD: BigDecimal
): Pair {
  let id = event.params.fromToken
    .toHexString()
    .concat("-")
    .concat(event.params.toToken.toHexString());
  ...
  pair.lastTradePrice = dealedFromAmount.gt(ZERO_BD)
    ? dealedToAmount.div(dealedFromAmount)
    : ZERO_BD;
  ...
  updateStatistics(
    event,
    pair as Pair,
    pair.volumeBaseToken,
    pair.volumeQuoteToken,
    ZERO_BD,
    ZERO_BD,
    ZERO_BD,
    ZERO_BD,
...
export function createPairDetail(
  pair: Pair,
  pmmState: ethereum.Tuple,
  updatedAt: BigInt
): PairDetail {
  let pairDetail = PairDetail.load(pair.id);

  if (pairDetail == null) {
    pairDetail = new PairDetail(pair.id);
    pairDetail.pair = pair.id;
  }
  //   let _pmmState = pmmState as DVM__getPMMStateResultStateStruct;
  pairDetail.i = pmmState[0].toBigInt();
  pairDetail.k = pmmState[1].toBigInt();
  pairDetail.b = pmmState[2].toBigInt();
  pairDetail.q = pmmState[3].toBigInt();
  pairDetail.b0 = pmmState[4].toBigInt();
  pairDetail.q0 = pmmState[5].toBigInt();
  pairDetail.r = BigInt.fromI32(pmmState[6].toI32());
  pairDetail.updatedAt = updatedAt;
  pairDetail.save();

  return pairDetail as PairDetail;
}
...
export function updateTokenTraderCount(
  tokenAddress: Address,
  userAddress: Address,
  event: ethereum.Event
): void {
  //todo
  let tokenTraderID = tokenAddress
    .toHexString()
    .concat("-")
...
export function updateUserDayDataAndDodoDayData(
  event: ethereum.Event,
  type: string
): void {
