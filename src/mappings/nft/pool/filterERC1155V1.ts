...
import {ChangeNFTAmountRange, ChangeTokenIdMap} from "../../../types/nft/templates/FilterERC1155V1/FilterERC1155V1";
...
export function handleTargetOut(event: TargetOut): void {
    ...
    let filterNft = FilterNft.load(filterNftId);
    if (filterNft != null) {
        filterNft.amount = filterNft.amount.minus(event.params.amount)
        filterNft.createdAt = event.block.timestamp;
        filterNft.updatedAt = event.block.timestamp;
        filterNft.save();
    }
