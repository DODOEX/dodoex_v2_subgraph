...
import { User, vDODO, DODO } from "../../types/vdodo/schema";
...
export function initTokenInfo(): void {
  ...
  let DODOAddress = Address.fromString(
  ...
  if (dodo == null) {
    ...
    dodo.updatedAt = ZERO_BI;
  }
  vdodo.dodoBalance = convertTokenToDecimal(
    fetchTokenBalance(DODOAddress, vDODOAddress),
    dodo.decimals
  );
