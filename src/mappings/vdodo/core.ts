...
export function handleMintVDODO(event: MintVDODO): void {
  ...
  if (history == null) {
    history = new MintHistory(
      event.transaction.hash
        .toHexString()
        .concat("-")
        .concat(event.logIndex.toString())
    );
    history.amount = convertTokenToDecimal(
      event.params.mintDODO,
      dodo.decimals
    );
  ...
  let operation = UserOperationHistory.load(
  ...
  let superiorUserInfo = fetchUserInfo(
    Address.fromString(user.superior.toHexString())
  );
  let superior = createUser(
    Address.fromString(user.superior.toHexString()),
    event
  );

  let superiorCreditChange = convertTokenToDecimal(
    superiorUserInfo.value3,
    dodo.decimals
  ).minus(superior.credit);

  superior.stakingPower = superiorUserInfo.value0;
  superior.superiorSP = superiorUserInfo.value1;
  superior.credit = convertTokenToDecimal(
    superiorUserInfo.value3,
    dodo.decimals
  );
  superior.superior = superiorUserInfo.value2;
  superior.spFromInvited = superior.spFromInvited.plus(superiorSpChange);
  superior.creditFromInvited =
    superior.creditFromInvited.plus(superiorCreditChange);

  operation.superiorSpChange = superiorSpChange;
  operation.superiorCreditChange = superiorCreditChange;
  operation.amount = operation.amount.plus(
    convertTokenToDecimal(event.params.mintDODO, dodo.decimals)
  );

  vdodo.mintAmount = vdodo.mintAmount.plus(
    convertTokenToDecimal(event.params.mintDODO, dodo.decimals)
  );
  user.mintAmount = user.mintAmount.plus(
    convertTokenToDecimal(event.params.mintDODO, dodo.decimals)
  );
  user.creditOfSuperior = user.creditOfSuperior.plus(superiorCreditChange);
  vdodo.dodoBalance = convertTokenToDecimal(
    fetchTokenBalance(Address.fromString(DODO_ADDRESS), dataSource.address()),
    dodo.decimals
  );
...
export function handleRedeemVDODO(event: RedeemVDODO): void {
  ...
  let history = RedeemHistory.load(
    event.transaction.hash
      .toHexString()
      .concat("-")
      .concat(event.logIndex.toString())
  ...
  if (history == null) {
    ...
    history.recieve = convertTokenToDecimal(
      event.params.receiveDODO,
      dodo.decimals
    );
  ...
  operation.superiorCreditChange = superiorCreditChange;
  operation.amount = operation.amount.plus(
    convertTokenToDecimal(event.params.receiveDODO, dodo.decimals)
  );

  vdodo.redeemAmount = vdodo.redeemAmount.plus(
    convertTokenToDecimal(event.params.receiveDODO, dodo.decimals)
  );
  user.redeemRecieveAmount = user.redeemRecieveAmount.plus(
    convertTokenToDecimal(event.params.receiveDODO, dodo.decimals)
  );
  user.redeemFeeAmount = user.redeemFeeAmount.plus(
    convertTokenToDecimal(event.params.feeDODO, dodo.decimals)
  );
  user.redeemBurnAmount = user.redeemBurnAmount.plus(
    convertTokenToDecimal(event.params.burnDODO, dodo.decimals)
  );
  user.creditOfSuperior = user.creditOfSuperior.plus(superiorCreditChange);
  vdodo.dodoBalance = convertTokenToDecimal(
    fetchTokenBalance(Address.fromString(DODO_ADDRESS), dataSource.address()),
  ...
  vdodo.totalStakingPower = fetchTotalSp();
  vdodo.feeAmount = vdodo.feeAmount.plus(
    convertTokenToDecimal(event.params.feeDODO, dodo.decimals)
  );
  vdodo.burnAmount = vdodo.feeAmount.plus(
    convertTokenToDecimal(event.params.burnDODO, dodo.decimals)
  );
...
export function handlePreDeposit(event: PreDeposit): void {
  ...
  let dodo = DODO.load(DODO_ADDRESS);
  if (dodo == null) return;
  vdodo.totalBlockReward = vdodo.totalBlockReward.plus(
    convertTokenToDecimal(event.params.dodoAmount, dodo.decimals)
  );
  vdodo.updatedAt = event.block.timestamp;
  vdodo.save();
}
...
export function handleChangePerReward(event: ChangePerReward): void {
  ...
  vdodo.dodoPerBlock = convertTokenToDecimal(
    event.params.dodoPerBlock,
    dodo.decimals
  );
  vdodo.updatedAt = event.block.timestamp;
  vdodo.save();
...
export function handleTransfer(event: Transfer): void {
  ...
  let fromUserSuperior = createUser(
    Address.fromString(fromUser.superior.toHexString()),
    event
  );
  let toUserSuperior = createUser(
    Address.fromString(toUser.superior.toHexString()),
    event
  );
  let fromUserSuperiorInfo = fetchUserInfo(
    Address.fromString(fromUser.superior.toHexString())
  );
  let toUserSuperiorInfo = fetchUserInfo(
    Address.fromString(toUser.superior.toHexString())
  );
  let spChangeFromUserSuperior = fromUserSuperiorInfo.value0.minus(
    fromUserSuperior.stakingPower
  );
  let spChangeToUserSuperior = toUserSuperiorInfo.value0.minus(
    toUserSuperior.stakingPower
  );
  let creditChangeFromUserSuperior = convertTokenToDecimal(
    fromUserSuperiorInfo.value3,
    dodo.decimals
  ).minus(fromUserSuperior.credit);
  let creditChangeToUserSuperior = convertTokenToDecimal(
    toUserSuperiorInfo.value3,
    dodo.decimals
  ).minus(toUserSuperior.credit);

  fromUserSuperior.stakingPower = fromUserSuperiorInfo.value0;
  fromUserSuperior.credit = convertTokenToDecimal(
    fromUserSuperiorInfo.value3,
    dodo.decimals
  );
  fromUserSuperior.spFromInvited = fromUserSuperior.spFromInvited.plus(
    spChangeToUserSuperior
  );
  fromUserSuperior.creditFromInvited = fromUserSuperior.creditFromInvited.plus(
    creditChangeFromUserSuperior
  );

  toUserSuperior.stakingPower = toUserSuperiorInfo.value0;
  toUserSuperior.credit = convertTokenToDecimal(
    toUserSuperiorInfo.value3,
    dodo.decimals
  );
  toUserSuperior.spFromInvited = toUserSuperior.spFromInvited.plus(
    spChangeFromUserSuperior
  );
  toUserSuperior.creditFromInvited = toUserSuperior.creditFromInvited.plus(
    creditChangeToUserSuperior
  );

  fromUser.creditOfSuperior = fromUser.creditOfSuperior.plus(
    creditChangeFromUserSuperior
  );
  fromUser.redeemRecieveAmount = fromUser.redeemRecieveAmount.plus(
    convertTokenToDecimal(event.params.amount, vdodo.decimals).times(
      BigDecimal.fromString("100")
    )
  );
  toUser.creditOfSuperior = toUser.creditOfSuperior.plus(
    creditChangeToUserSuperior
  );
  toUser.mintAmount = toUser.mintAmount.plus(
    convertTokenToDecimal(event.params.amount, vdodo.decimals).times(
      BigDecimal.fromString("100")
    )
  );
  ...
  let transferHistoryID = event.transaction.hash
    .toHexString()
    .concat("-")
    .concat(event.logIndex.toString());
  ...
  if (transferHistory == null) {
    ...
    transferHistory.amount = convertTokenToDecimal(
      event.params.amount,
      vdodo.decimals
    );
  }
...
export function handleDonateDODO(event: DonateDODO): void {
  ...
  let donateHistoryID = event.transaction.hash
    .toHexString()
    .concat("-")
    .concat(event.logIndex.toString());
  ...
  if (donateHistory == null) {
    ...
    donateHistory.dodoAmount = convertTokenToDecimal(
      event.params.donateDODO,
      dodo.decimals
    );
